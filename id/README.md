...
**返回值：**
- `IDGenerator` — 生成器实例
- `error` — 创建失败时返回错误（如 machineId 函数报错）
...
- **机器 ID 唯一性是调用方责任**：本包不做跨进程的机器 ID 唯一性校验。集群中每个运行中的生成器必须使用不同的机器 ID，否则可能生成重复 ID。推荐使用以下方案协调唯一性：
  - **配置分配表**：部署时由配置中心或运维系统为每个实例分配唯一机器 ID
  - **ETCD / Consul**：实例启动时通过分布式 KV 存储注册并获取唯一机器 ID
  - **Redis INCR**：使用 Redis 原子递增命令分配机器 ID
- **机器 ID 范围**：默认 16 bits，取值范围 0-65535
