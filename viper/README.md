...
- 支持配置文件动态监听
- 配置文件热重载成功后通过 `Watch()` 通知调用方
- 动态监听期间配置读取或解析失败后通过 `Errors()` 通知调用方，并保留旧配置
...
首次或重复调用 `LoadConfig()` 失败时，错误会直接同步返回，不会写入 `Errors()` 通道。
...
返回配置文件成功热重载的状态通知通道。首次或重复调用 `LoadConfig()` 成功本身不会发送 `Watch()` 通知。通道容量为 1；消费方尚未读取通知时，后续成功热重载通知会被合并。
...
- `Watch()` 只通知成功的热重载；首次或重复调用 `LoadConfig()` 成功不会触发通知。
